...
function getPriceConfig(productConfig, interval) {
  ...
  if (len !== 1) {
    if (len !== 0) {
      if (interval !== undefined) {
        return Exn.raiseError("Product \"" + productConfig.name + "\" has multiple prices for interval \"" + interval + "\"");
      } else {
        return Exn.raiseError("Product \"" + productConfig.name + "\" has multiple prices but no interval specified. Use \"interval\" param to dynamically choose which price use for the plan");
      }
    } else if (interval !== undefined) {
      return Exn.raiseError("Product \"" + productConfig.name + "\" doesn't have prices for interval \"" + interval + "\"");
    } else {
      return Exn.raiseError("Product \"" + productConfig.name + "\" doesn't have any prices");
    }
  } else {
    return match$1[0];
  }
...
async function syncProduct(stripe, productConfig, meters, usedCustomerMeters, interval) {
  ...
  if (len !== 1) {
    if (len !== 0) {
      product = Exn.raiseError("There are multiple active products \"" + productConfig.ref + "\". Please go to dashboard and delete not needed ones (https://dashboard.stripe.com/test/products?active=true)");
    } else {
  ...
  if (prices.has_more) {
    Exn.raiseError("The pagination on prices is not supported yet. Product \"" + productConfig.ref + "\" has to many active prices");
  }
  let createPriceFromConfig = async priceConfig => {
    ...
    if (match !== undefined) {
      if (match.TAG === "Metered") {
        let ref = match.ref;
        let meters$1 = meters !== undefined ? meters : Exn.raiseError("The \"meters\" argument is required when product catalog contains a Metered price");
        let usedCustomerMeters$1 = usedCustomerMeters !== undefined ? Primitive_option.valFromOption(usedCustomerMeters) : Exn.raiseError("The \"usedCustomerMeters\" argument is required when product catalog contains a Metered price");
  ...
  let existingPrice = prices.data.find(price => {
    ...
    let usedCustomerMeters$1 = usedCustomerMeters !== undefined ? Primitive_option.valFromOption(usedCustomerMeters) : Exn.raiseError("The \"usedCustomerMeters\" argument is required when product catalog contains a Metered price");
    if (!(match$2.usage_type === "metered" && match$2.interval === match$3.interval && Option.isSome(Primitive_option.fromNull(match$2.meter)) && price.metadata["#meter_ref"] === match$3.ref)) {
      return false;
    }
...
async function findByMetadata(stripe, metadata) {
  ...
  let lookup = async startingAfter => {
    ...
    if (match !== undefined) {
      ...
      return await lookup(lastUnsafe(page.data).id);
    } else {
      return;
    }
  };
  ...
  if (items.length !== 0) {
    ...
    if (cache.hasMore) {
      ...
      let c = await lookup(lastUnsafe(cache.items).id);
...
function getMeterId(subscription, meterRef) {
  return Option.flatMap(Option.flatMap(subscription.items.data.find(item => item.price.metadata["#meter_ref"] === meterRef), i => Primitive_option.fromNull(i.price.recurring)), r => Primitive_option.fromNull(r.meter));
}

function getMeterEventName(subscription, meterRef) {
  return Option.flatMap(subscription.items.data.find(item => item.price.metadata["#meter_ref"] === meterRef), i => i.price.metadata["#meter_event_name"]);
}
...
async function listSubscriptions(stripe, config, customerId) {
  ...
  if (match.has_more) {
    return Exn.raiseError("Found more than 100 subscriptions, which is not supported yet");
  } else {
...
function calculatePastUsageBill(priceAmount, startedAt, now, interval) {
  ...
  if (interval !== undefined) {
    switch (interval) {
      case "day" :
      case "week" :
        monthlyPrice = Exn.raiseError("The past usage bill only supports subscriptions with yearly or monthly intervals");
        break;
      case "month" :
  ...
  let totalMonths = Math.imul(yearsDiff, 12) + monthsDiff | 0;
...
function processData(data, config) {
  ...
  if (customerLookupFields.length === 0) {
    Exn.raiseError("The data schema must define at least one primary field with ~customerLookup=true");
  }
...
async function createHostedCheckoutSession(stripe, params) {
  ...
  if (products.length !== 0) {
    ...
    if (match !== undefined) {
      ...
      products$1 = $$Array.filterMap(products, p => {
    ...
    products$1 = Exn.raiseError("Plan \"" + planId + "\" doesn't have any products configured");
  }
  ...
  if (subscription !== undefined) {
    Exn.raiseError("There's already an active \"" + params.config.ref + "\" subscription for " + data.primaryFields.map(name => name + "=" + data.dict[name]).join(", ") + " with the \"" + subscription.metadata[planField] + "\" plan and id \"" + subscription.id + "\". Either update the existing subscription or cancel it and create a new one");
  } else {
