  ...
  return async (stripe, metadata) => {
    ...
    let catchUpToCache = async endingBefore => {
      ...
      if (data.length === 10000) {
        Stdlib_JsError.throwWithMessage(`Too many new ` + name + `s to cache.`);
      }
...
function getPriceConfig(productConfig, interval) {
  ...
  if (len !== 1) {
    if (len !== 0) {
      if (interval !== undefined) {
        return Stdlib_JsError.throwWithMessage(`Product "` + productConfig.name + `" has multiple prices for interval "` + interval + `"`);
      } else {
        return Stdlib_JsError.throwWithMessage(`Product "` + productConfig.name + `" has multiple prices but no interval specified. Use "interval" param to dynamically choose which price use for the plan`);
      }
    } else if (interval !== undefined) {
      return Stdlib_JsError.throwWithMessage(`Product "` + productConfig.name + `" doesn't have prices for interval "` + interval + `"`);
    } else {
      return Stdlib_JsError.throwWithMessage(`Product "` + productConfig.name + `" doesn't have any prices`);
    }
  } else {
    return match$1[0];
  }
...
async function syncProduct(stripe, productConfig, meters, usedCustomerMeters, interval) {
  ...
  if (prices.has_more) {
    Stdlib_JsError.throwWithMessage(`The pagination on prices is not supported yet. Product "` + productConfig.ref + `" has to many active prices`);
  }
  let createPriceFromConfig = async priceConfig => {
    ...
    if (match !== undefined) {
      if (match.TAG === "Metered") {
        ...
        let usedCustomerMeters$1 = usedCustomerMeters !== undefined ? usedCustomerMeters : Stdlib_JsError.throwWithMessage(`The "usedCustomerMeters" argument is required when product catalog contains a Metered price`);
...
function constructEvent(stripe, body, sig, secret) {
  try {
    ...
    if (err.RE_EXN_ID === "JsExn") {
      return {
        TAG: "Error",
        _0: Stdlib_JsExn.message(err._1)
      };
    }
    throw err;
  }
...
async function listSubscriptions(stripe, config, customerId) {
  ...
  if (match.has_more) {
    return Stdlib_JsError.throwWithMessage(`Found more than 100 subscriptions, which is not supported yet`);
  } else {
...
function calculatePastUsageBill(priceAmount, startedAt, now, interval) {
  ...
  if (interval !== undefined) {
    switch (interval) {
      case "day" :
      case "week" :
        monthlyPrice = Stdlib_JsError.throwWithMessage("The past usage bill only supports subscriptions with yearly or monthly intervals");
        break;
      case "month" :
...
function processData(data, config) {
  ...
  if (customerLookupFields.length === 0) {
    Stdlib_JsError.throwWithMessage("The data schema must define at least one primary field with ~customerLookup=true");
  }
  ...
  let s = subscriptions.find(subscription => {
    if (usedMetersAcc !== undefined) {
      subscription.items.data.forEach(item => {
        ...
        if (meterEventName !== undefined) {
          usedMetersAcc.add(meterEventName);
          return;
        }
      });
    }
...
async function createHostedCheckoutSession(stripe, params) {
  ...
  if (products.length !== 0) {
    ...
    products$1 = Stdlib_JsError.throwWithMessage(`Plan "` + planId + `" doesn't have any products configured`);
  }
  let customer = await findOrCreateByMetadata(stripe, data.customerMetadata);
  let usedCustomerMeters = new Set();
  let subscription = await internalRetrieveSubscription(stripe, data, params.config, customer.id, usedCustomerMeters);
  if (subscription !== undefined) {
    Stdlib_JsError.throwWithMessage(`There's already an active "` + params.config.ref + `" subscription for ` + data.primaryFields.map(name => name + `=` + data.dict[name]).join(", ") + ` with the "` + subscription.metadata[planField] + `" plan and id "` + subscription.id + `". Either update the existing subscription or cancel it and create a new one`);
  } else {
