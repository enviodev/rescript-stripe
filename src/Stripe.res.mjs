...
async function syncProduct(stripe, productConfig, meters, usedCustomerMeters, interval) {
  ...
  let prices$1 = await Promise.all(productConfig.prices.filter(priceConfig => {
    ...
    let existingPrice = prices.data.find(price => {
      ...
      let usedCustomerMeters$1 = usedCustomerMeters !== undefined ? Primitive_option.valFromOption(usedCustomerMeters) : Exn.raiseError("The \"usedCustomerMeters\" argument is required when product catalog contains a Metered price");
      if (!(priceRecurring.usage_type === "metered" && priceRecurring.interval === match$2.interval && Option.isSome(Primitive_option.fromNull(priceRecurring.meter)) && price.metadata["#meter_ref"] === match$2.ref)) {
        return false;
      }
...
async function createHostedCheckoutSession(stripe, params) {
  ...
  let products = p.length !== 0 ? p : Exn.raiseError("Tier \"" + planId + "\" doesn't have any products configured");
  ...
  let session = await stripe.checkout.sessions.create({
    ...
    line_items: productItems.map(param => {
      ...
      let lineItemPrice = len !== 1 ? (
          len !== 0 ? (
              match !== undefined ? Exn.raiseError("Product \"" + product.name + "\" has multiple prices for interval \"" + match + "\"") : Exn.raiseError("Product \"" + product.name + "\" has multiple prices but no interval specified. Use \"interval\" param to dynamically choose which price use for the plan")
            ) : (
              match !== undefined ? Exn.raiseError("Product \"" + product.name + "\" doesn't have prices for interval \"" + match + "\"") : Exn.raiseError("Product \"" + product.name + "\" doesn't have any prices")
            )
        ) : prices[0];
